...
## Contacts / AboutApp / AboutCompany / Help

| Вызов | Файл фронта | Бэкенд (`azerpetrol-topaz-server`) | Что делает |
|---|---|---|---|
| `POST get_contacts/` (`{token, contacts}` в JSON body, НЕ через `axiosIntsanse`) | `features/Contacts/MapContacts/api/contactsApi.ts` | `get_contacts/index.php` → `port_in_mobile_clients_test()` → `select_mobile_clients_test()` | Отправляет ВСЮ телефонную книгу пользователя на сервер, получает обратно только тех, чей номер зарегистрирован в `clients`. **Найден и исправлен критический фронт-баг (2026-09-20, коммит `7e82fa8`, исключение из правила «фронт не трогаем» по явному согласию пользователя)**: константа `DOMEN` указывала на чужой домен вендора-первоисточника (`21vek.azs-control.ru` — демо/шаблон, из которого изначально скопировано приложение), контакты пользователя реально уходили туда, а не на наш сервер, при каждой выдаче разрешения. Заменено на `topaz.poteryashka.pro`. Заодно на исправленном домене найден и исправлен **бэк-баг** (коммит `2a95979` в `azerpetrol-topaz-server`): `clients.phone` — `decimal(15,0)`, нечисловая/пустая строка из контакта молча коэрцировалась MySQL в `0` и ложно матчилась с реальной «битой» строкой в БД (`client id=59690`, `phone=0`, с 2024 года) — контакт с нечисловым/пустым номером ложно показывался как «зарегистрированный пользователь». Добавлена валидация формата номера перед запросом. Оба фикса подтверждены живым end-to-end тестом на синтетических данных (curl на исправленный домен, до и после фикса). |
| `GET article/?id=` | `features/GetArticle/api/getArticleApi.ts` (общий для AboutApp/AboutCompany/Help, разные `id`) | `article/index.php` → `port_in_mobile_articles()` → `mobile_articles` | Три статичные статьи по `id` (1=О компании, 2=О приложении, 3=Помощь). Проверено живым SQL: все три строки существуют, с заголовками и непустым `html_text` — не заглушки. Багов не найдено. |

Фронтенд-находка в бэклог (не фикс, т.к. не бэк и не критично): вероятная гонка `MapContacts`/`checkIsContactInSystem` вокруг AsyncStorage — см. `2026-09-18-screen-review-frontend-bugs-backlog.md`.

## Остальные экраны — не разобраны

Products (пропущен по просьбе пользователя — там изменения в разработке),
Settings, DeleteAccount и т.д. — эндпоинты добавятся сюда по мере прохода.
